...
import { getConnectedRepos, getTodayUpdates, type ConnectedRepo, type TodayUpdateRepo } from "../api/repoApi";
      ...
      <main className={styles.main}>
        ...
        <div className={styles.statRow}>
          ...
          <div className={styles.statCard}>
            ...
            <div className={styles.statValue}>{reposLoading ? "-" : repos.filter((r) => r.blog).length}</div>
        ...
        <div className={styles.section}>
          ...
          >
            <div className={styles.sectionHeaderLeft}>
              ...
              <span className={styles.sectionCount}>{repos.filter((r) => r.blog).length}개 레포</span>
            </div>
            <div className={styles.sectionHeaderRight}>
              <button
                className={styles.headerActionBtn}
                onClick={(e) => { e.stopPropagation(); navigate("/blogSettings"); }}
              >
                <svg
                  width="12"
                  height="12"
          ...
          >
            <div>
              {reposLoading ? (
                ...
                <div className={styles.emptyText}>블로그로 연동된 레포지토리가 없습니다.</div>
              ) : (
                repos
                  .filter((r) => r.blog)
                  .map((br) => {
                    const langColor = LANG_COLORS[br.language ?? ""] ?? "#8b949e";
                    return (
                      <div key={br.githubRepoId} className={styles.blogRepoRow}>
                        <div className={styles.blogRepoIcon}>
                          <svg width="14" height="14" viewBox="0 0 16 16" fill="#a371f7">
                            <path d="M0 1.75A.75.75 0 01.75 1h4.253c1.227 0 2.317.59 3 1.501A3.744 3.744 0 0111.006 1h4.245a.75.75 0 01.75.75v10.5a.75.75 0 01-.75.75h-4.507a2.25 2.25 0 00-1.591.659l-.622.621a.75.75 0 01-1.06 0l-.622-.621A2.25 2.25 0 005.258 13H.75a.75.75 0 01-.75-.75zm7.251 10.324l.004-5.073-.002-2.253A2.25 2.25 0 005.003 2.5H1.5v9h3.757a3.75 3.75 0 012 .756zM8.755 4.75l-.004 7.322a3.752 3.752 0 012-.572H14.5v-9h-3.495a2.25 2.25 0 00-2.25 2.25z" />
                          </svg>
                        </div>
                        <div className={styles.blogRepoInfo}>
                          <div className={styles.blogRepoName}>{br.name}</div>
                          <div className={styles.blogRepoSub}>{br.description ?? ""}</div>
                        </div>
                        <div className={styles.blogRepoRight}>
                          ...
                          <span className={styles.repoUpdated}>{toRelativeTime(br.pushedAt)}</span>
                        </div>
                      </div>
                    );
                  })
          ...
          >
            <div>
              {updatesLoading ? (
                ...
                <div className={styles.emptyText}>오늘 업데이트된 레포지토리가 없습니다.</div>
              ) : (
                    ...
                    <div key={ur.repoName} className={styles.updateRepoGroup}>
                      ...
                      >
                        <div className={styles.updateRepoIcon}>
                          <svg width="13" height="13" viewBox="0 0 16 16" fill={langColor}>
                            <path d="M2 2.5A2.5 2.5 0 014.5 0h8.75a.75.75 0 01.75.75v12.5a.75.75 0 01-.75.75h-2.5a.75.75 0 010-1.5h1.75v-2h-8a1 1 0 00-.714 1.7.75.75 0 01-1.072 1.05A2.495 2.495 0 012 11.5v-9zm10.5-1V9h-8c-.356 0-.694.074-1 .208V2.5a1 1 0 011-1h8z" />
                          </svg>
                        </div>
                        <span className={styles.updateRepoName}>{ur.repoName}</span>
                        ...
                        <span className={styles.updateFileCount}>{ur.files.length}개 파일</span>
                        <svg
                          className={cx(styles.chevron, updateOpen[i] && styles.open)}
                          viewBox="0 0 16 16"
                          fill="none"
                          stroke="#8b949e"
                          strokeWidth="1.8"
                      ...
                      <div className={cx(styles.updateFiles, !updateOpen[i] && styles.collapsed)}>
                        {ur.files.map((f) => (
                          <div key={f.filePath} className={styles.updateFileItem}>
                            <span className={cx(styles.fileTypeDot, dotClass(f.changeType as FileType))}></span>
                            <span className={styles.fileName}>{f.filePath}</span>
                            <span className={styles.fileMsg}>{f.commitMessage}</span>
                            <div className={styles.fileDiff}>
                              <span className={styles.diffAdd}>+{f.additions}</span>
                              <span style={{ color: "#484f58" }}>/</span>
                              <span className={styles.diffDel}>−{f.deletions}</span>
                            </div>
